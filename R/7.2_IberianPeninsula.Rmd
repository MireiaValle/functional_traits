...
```
##We filter the data frame to the extent we are interested in: (ext= extent (c(-11,0, 42, 46))
```{r setup}
zoom_base_df1 <- filter(base_df, between(base_df$long, -11, 0))
zoom_base_df <- filter(zoom_base_df1, between(zoom_base_df1$lat, 42, 46))
```
...
unique (zoom_base_df$loiczid) %>% length ()#640
...
unique (zoom_species_hs$loiczid) %>% length() #352
unique (zoom_species_hs$am_sid) %>% length()#1214
...
unique (zoom_species_hs_subset$am_sid) %>% length()#1210
...
```{r setup}
zoom_basemap <- raster (ext= extent (c(-11,5, 35, 45)), res=0.5)
...
table (zoom_Loo$n_distinct(Loo_groups))
