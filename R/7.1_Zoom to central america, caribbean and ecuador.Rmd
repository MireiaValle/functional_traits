---
title: "7.1_Zoom to central america, caribbean and ecuador"
author: "Mireia Valle"
date: "10/11/2019"
...
```
##We filter the data frame to the extent we are interested in: (ext= extent (c(-95,-85, -3, 13))
```{r setup}
zoom_base_df1 <- filter(base_df, between(base_df$long, -95, -80))
zoom_base_df <- filter(zoom_base_df1, between(zoom_base_df1$lat, -3, 13))
```
...
unique (zoom_base_df$loiczid) %>% length ()#960
...
unique (zoom_species_hs$loiczid) %>% length() #928
unique (zoom_species_hs$am_sid) %>% length()#3780
head (zoom_species_hs)
colnames (zoom_species_hs)
str (zoom_species_hs)
```
...
unique (zoom_species_hs_subset$am_sid) %>% length()#3780
...
#we save the data
#write.csv (zoom_spread_loiczid_spp, file = "zoom_assemblage.csv")
```
#------------------------------------------------------------------------------------- #
#MAPPING THE DISTRIBUTION OF ALL SPECIES IN THE SELECTED EXTEND (3799 spp)
#------------------------------------------------------------------------------------- #
```{r setup}
...
```{r setup}
zoom_basemap <- raster (ext= extent (c(-95,-80, -3, 13)), res=0.5)
...
plot (zoom_EnvTemp_map, col=rainbow(6))
```
#------------------------------------------------------------------------------------- #
...
plot (zoom_Loo_map, col=rainbow(4))
...
plot (eel_map,
      ...
      axes=FALSE, zlim=c(0,100), col=rainbow(5))
plot (elon_map,
...
unique (zoom_traits$am_sid) %>% length()#3780
head (zoom_traits)
...
str (zoom_traits_ok)
#we save the data
write.csv (zoom_traits_ok, file = "zoom_traits_ok.csv")
...
#save the data
write.csv (zoom_assemblage_ok, file = "zoom_assemblage_ok.csv")
...
zoom_traits_ok_NA <- select (zoom_traits_ok, -"FeedingType", -"bodyshape", -"Resilience", -"pd50", -"depthrangedeep", -"importance", -"pricecateg", -"IUCN_Code", -"vulnerability", -"EnvTemp")
summary (zoom_traits_ok_NA)

zoom_traits_ok_NA_clear <- select (zoom_traits_ok_NA, -"class", -"order", -"family")
summary (zoom_traits_ok_NA_clear)

#we save the data
write.csv (zoom_traits_ok_NA_clear, file = "zoom_traits_ok_NA_clear.csv")

qual_funct_space_clear<-quality_funct_space(zoom_traits_ok_NA_clear, traits_weights=NULL, nbdim=10, metric="Gower", dendro=TRUE, plot="quality_funct_space_MarineFish_clear")
...
coord_fish_4D<-qual_funct_space_clear$details_funct_space$mat_coord[,1:4]
...
plot_funct_space( coord_fish_4D,col_sp="blue", pch_sp=21, nm_jpeg="Fspace_fish_4D.jpeg", cex_sp=1.6, close_jpeg=TRUE   )
...
as.tibble (zoom_assemblage_ok)
zoom_assemblage_ok_100 <- zoom_assemblage_ok*100
matrix_zoom_assemblage_ok_100 <- data.matrix(zoom_assemblage_ok_100)

FD_assemblages<-multidimFD(coord_fish_5D, matrix_zoom_assemblage_ok_100 , check_species_pool=TRUE, verb=TRUE,
                      nm_asb_plot=row.names(matrix_zoom_assemblage_ok_100), folder_plot= paste(my_path,"/results/plot_FD", sep=""),
                      Faxes_plot=colnames(coord_fish_5D)[1:4], Faxes_nm_plot=colnames(coord_fish_5D)[1:4],
